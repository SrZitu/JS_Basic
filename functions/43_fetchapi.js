// The Fetch API is a modern JavaScript API for making network requests, such as fetching resources from a server.
//It is a simpler and more powerful alternative to the older XMLHttpRequest(XHR) API, and it uses promises to handle asynchronous responses.
...
// Handle the response: Use the.then() method to handle the response from the server.
//You can parse the response as JSON, text, or a blob using the.json(), .text(), or.blob() methods, respectively.
...
// In the example above, we use the fetch() function to make a GET request to the URL 'https://example.com/api/data'.
//We then use the.json() method to parse the response as JSON, and log the resulting data to the console.If an error occurs during the fetch request, we catch it and log the error to the console.

// The Fetch API can be used to make a variety of network requests, including GET and POST requests, and it supports a wide range of data formats,
// including JSON, XML, and plain text.It is widely supported by modern browsers and is a powerful tool for building dynamic and responsive web applications.


// Fetch requests can include various options, such as headers, request method(GET, POST, etc.), mode(cors, no - cors, same - origin), and more.
//These options are passed as an object to the fetch() function as the second argument.

// The Fetch API can be used with any HTTP method, including GET, POST, PUT, DELETE, and more.To use a method other than GET,
// you need to include the method in the options object when calling fetch().
